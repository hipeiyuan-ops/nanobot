...
运行后pip 会**自动在你的 Python 环境里创建一个启动脚本，它的真实位置在这里：你的Python环境目录\Scripts\nanobot.exe**

它是一个**超级小的启动器，实际运行的是nanobot.cli.commands:app**
...
，用 uv 这个新一代 Python 包管理器，全局、隔离、可直接调用地安装 nanobot-ai 这个 AI 助手工具
...
运行这个，然后手动去改`~/.nanobot/config.json`
...
> `allowFrom`: Add your open\_id (find it in nanobot logs when you message the bot). Use `["*"]` to allow all users.
...
See [docs/PYTHON\_SDK.md](docs/PYTHON_SDK.md) for the full SDK reference.
